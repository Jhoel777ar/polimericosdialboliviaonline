...
<template>
    ...
    <div
        ...
        <div
            ...
            <div
                class="precompra-container px-6 py-10 md:px-20 bg-gradient-to-b from-gray-950 via-gray-900 to-gray-800 min-h-screen">
                <h2
                    class=" md:text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-500 via-blue-500 to-indigo-600 text-center mb-12 drop-shadow-lg">
                    {{ props.producto?.nombre }}
                </h2>
                <div class="flex flex-col lg:flex-row gap-10">
                    <div class="lg:w-1/2 w-full flex justify-center">
                        <div
                            class="relative overflow-hidden rounded-3xl shadow-2xl bg-gradient-to-b from-gray-800/80 to-gray-950/90 p-6 backdrop-blur-md border border-gray-700 group">
                            <img :src="'/storage/' + props.producto?.imagen_url" alt="Imagen del producto" class="w-full h-auto object-contain max-h-[500px] rounded-xl shadow-lg 
                transition-transform duration-300 ease-in-out transform 
                group-hover:scale-150 group-hover:cursor-zoom-in" />
                            <div
                                class="absolute top-4 right-4 bg-gray-900/70 p-2 rounded-full text-white opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                                <i class="fas fa-search-plus text-xl"></i>
                            </div>
                        </div>
                    </div>
                    <div class="lg:w-1/2 w-full flex flex-col space-y-8">
                        <div class="glass-card bg-gray-800/70 backdrop-blur-xl shadow-2xl p-6 rounded-2xl">
                            <h3 class="text-2xl font-semibold text-gray-100 mb-4">Descripción</h3>
                            <p class="text-base text-gray-300">{{ props.producto?.descripcion }}</p>
                        </div>
                        <div class="glass-card bg-gray-800/70 backdrop-blur-xl shadow-2xl p-6 rounded-2xl">
                            ...
                            <ul class="text-base text-gray-400 space-y-3">
                        ...
                        <div v-if="props.producto.activo === 0"
                            class="glass-card bg-red-800/70 backdrop-blur-lg shadow-2xl p-6 rounded-2xl">
                        ...
                        <div v-if="props.producto?.activo === 1">
                            <div class="flex flex-col gap-6">
                                <div class="glass-card bg-gray-800/70 backdrop-blur-xl shadow-2xl p-6 rounded-2xl">
                                    <label for="cantidad" class="text-base text-gray-300 font-semibold">Cantidad</label>
                                    <select id="cantidad"
                                        class="mt-3 p-3 w-full rounded-xl bg-gray-700 text-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500"
                                        v-model="cantidad" :disabled="props.producto?.stock <= 0"
                            ...
                            <button @click="agregarAlCarrito"
                                class="w-full py-4 rounded-2xl text-white font-semibold shadow-xl text-lg transition-all duration-300"
                                :class="{
                                    'bg-gradient-to-r from-green-500 to-blue-600 hover:from-green-400 hover:to-blue-500': props.producto?.stock > 0,
                                    'bg-gray-600 cursor-not-allowed': props.producto?.stock <= 0
            ...
            <div
                class="precompra-container px-6 py-10 md:px-20 bg-gradient-to-br from-gray-900 via-black to-gray-800 shadow-2xl rounded-2xl">
                <div class="productos-relacionados-container mt-16">
                    <h3
                        class="text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-teal-400 to-green-400 mb-8 text-center">
                        Productos Relacionados
                    </h3>
                    ...
                    <div v-else class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 px-4">
                        <div v-for="relacionado in productosPorPagina" :key="relacionado.id" class="relative bg-gradient-to-br from-gray-950/90 via-gray-900/90 to-black/90 
           backdrop-blur-xl rounded-2xl p-6 shadow-xl border border-gray-800/50 
           group hover:shadow-2xl hover:border-gray-700/70 
           transform transition-all duration-300 hover:-translate-y-2">
                            <img :src="'/storage/' + relacionado?.imagen_url" alt="Imagen de producto relacionado"
                                class="w-full h-48 object-contain mb-6 rounded-xl shadow-md 
                transition-transform duration-300 group-hover:scale-105" />
                            <div v-if="relacionado?.stock === 0" class="absolute top-2 left-2 bg-gradient-to-r from-red-600/90 to-red-400/90 
                text-white text-sm font-semibold px-4 py-1 rounded-full flex items-center gap-2 
                shadow-md backdrop-blur-sm">
                                <i class="fas fa-ban text-red-200"></i> Agotado
                            </div>
                            <div class="space-y-2">
                                <h4 class="text-2xl font-bold text-white flex items-start gap-2 break-words max-w-full">
                                ...
                                <p class="text-sm text-gray-400 flex items-start gap-2 break-words max-w-full">
                                    ...
                                    <span class="flex-1 break-words overflow-hidden">{{ relacionado?.color.color
                                    }}</span>
                                </p>
                            </div>
                            <button @click="redirectToPrecompra(relacionado)" class="mt-6 w-full px-6 py-3 bg-gradient-to-r from-teal-600 to-blue-600 
             text-white rounded-xl font-medium shadow-md 
             flex items-center justify-center gap-2
             transition-all duration-300 hover:from-teal-500 hover:to-blue-500 
             hover:shadow-xl hover:scale-105">
                ...
                opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-2xl pointer-events-none">
                    ...
                    <div class="flex justify-between mt-12">
                        <button
                            class="text-white bg-gradient-to-r from-blue-500 to-indigo-600 py-3 px-8 rounded-xl shadow-md hover:shadow-lg hover:from-blue-400 hover:to-indigo-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 transform hover:scale-105"
...
<style>
body {
    background: linear-gradient(45deg, #121212 20%, #2a2a35 50%, #3b4c5e 80%, #1f2a33 100%);
